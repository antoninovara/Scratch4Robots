...
class Robot():
  ...
  def get_laser_distance(self, laserDirection):
      """
     ...
     if laserDirection == "frontal":
           minLaser = 0
           maxLaser = 10
      elif laserDirection == "right":
           maxLaser = int (len (laser) / 4)
           minLaser = maxLaser - 10
      else
           minLaser = int (len (laser) * 3 / 4)
           maxLaser = minLaser + 10

      # clean data (unranged values, e.g. nan)
      l = [x for x in laser.values if str(x) != 'nan' and x >= minLaser and x < maxLaser]
